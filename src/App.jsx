...
const emptyForm = { date: today(), text: "", author: "", source: "", category: "명언", tags: "", imageUrl: "", imageFile: null };

const initialState = {
  quotes: [], search: "", filterCat: "",
...
function reducer(s, a) {
  switch (a.type) {
    ...
    case "RESET_FORM": return { ...s, form: emptyForm, editId: null, showForm: false };
    case "SHOW_FORM": return { ...s, showForm: true, editId: a.editId || null, form: a.form || emptyForm };
    case "SAVE_QUOTE": {
      ...
      return { ...s, quotes, showForm: false, editId: null, form: emptyForm };
    }
...
export default function App() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("quotes_v3");
      if (saved) dispatch({ type: "SET_QUOTES", quotes: JSON.parse(saved) });
    } catch {}
  }, []);

  useEffect(() => {
    try { localStorage.setItem("quotes_v3", JSON.stringify(s.quotes)); } catch {}
  }, [s.quotes]);

  useEffect(() => {
    if (!s.showForm) setImgPreview(null);
    else if (s.form.imageUrl) setImgPreview(null);
  }, [s.showForm]);
  ...
  const handleFile = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      dispatch({ type: "SET_FORM", fields: { imageUrl: dataUrl, imageFile: file } });
    };
  ...
  const startEdit = (q) => dispatch({
    ...
    form: { ...q, tags: q.tags?.join(", ") || "", imageFile: null }
  });

  const inp = { border: "2px solid #F4C0D1", borderRadius: 12, padding: "9px 12px", fontSize: 14, outline: "none", width: "100%", boxSizing: "border-box", background: "#fff" };
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #FFF0F5 0%, #F0F7FF 50%, #F0FFF8 100%)", fontFamily: "sans-serif" }}>
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "2rem 1rem" }}>
          ...
          <div key={date} style={{ marginBottom: "2rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {qs.map((q, i) => {
                ...
                return (
                  <div key={q.id}
                    style={{ background: c.bg, border: `2px solid ${c.border}`, borderRadius: 16, overflow: "hidden", cursor: "pointer" }}
                    onClick={() => setExpandId(expandId === q.id ? null : q.id)}
                  >
                    {q.imageUrl && (
                      <img src={q.imageUrl} alt="" style={{ width: "100%", maxHeight: 200, objectFit: "cover", display: "block" }} />
                    )}
                    <div style={{ padding: "1.1rem 1.25rem" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                        ...
                        <span style={{ background: CAT_BG[q.category] || "#f5f5f5", color: CAT_COLORS[q.category] || "#666", fontSize: 11, padding: "4px 10px", borderRadius: 20, whiteSpace: "nowrap", fontWeight: 700, flexShrink: 0 }}>
                          {q.category}
                        </span>
                      </div>
                      {(q.author || q.source) && (
        ...
        {s.showForm && (
          ...
          >
            <div style={{ background: "#fff", borderRadius: 20, border: "2px solid #F4C0D1", padding: "1.75rem", width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto" }}>
              <h2 style={{ margin: "0 0 1.25rem", fontSize: 18, fontWeight: 700, color: "#D4537E" }}>
                {s.editId ? "✏️ 어록 수정" : "✨ 새 어록 추가"}
              </h2>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <input type="date" style={{ ...inp }} value={s.form.date}
                  onChange={e => dispatch({ type: "SET_FORM", fields: { date: e.target.value } })} />
                <textarea style={{ ...inp, minHeight: 90, resize: "vertical", fontFamily: "sans-serif" }}
                  placeholder="어록 내용 *" value={s.form.text}
                  onChange={e => dispatch({ type: "SET_FORM", fields: { text: e.target.value } })} />
                <input style={{ ...inp, borderColor: "#B5D4F4" }} placeholder="누가 한 말 (출처)" value={s.form.author}
                  onChange={e => dispatch({ type: "SET_FORM", fields: { author: e.target.value } })} />
                <input style={{ ...inp, borderColor: "#B5D4F4" }} placeholder="책 / 영화 / 드라마 등 출처" value={s.form.source}
                  onChange={e => dispatch({ type: "SET_FORM", fields: { source: e.target.value } })} />
                <select style={{ ...inp, borderColor: "#9FE1CB", cursor: "pointer" }}
                  value={s.form.category}
                  onChange={e => dispatch({ type: "SET_FORM", fields: { category: e.target.value } })}>
                  {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
                <input style={{ ...inp, borderColor: "#CECBF6" }} placeholder="태그 (쉼표로 구분: 인생, 용기, ...)" value={s.form.tags}
                  onChange={e => dispatch({ type: "SET_FORM", fields: { tags: e.target.value } })} />

                {/* Image section */}
                <div style={{ border: "2px dashed #F4C0D1", borderRadius: 12, padding: "1rem", background: "#FFF8FB" }}>
                  <p style={{ margin: "0 0 8px", fontSize: 13, fontWeight: 700, color: "#D4537E" }}>🖼️ 이미지 추가 (선택)</p>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <button onClick={() => fileRef.current.click()}
                      style={{ padding: "8px 14px", background: "#fff", border: "1.5px solid #F4C0D1", borderRadius: 20, cursor: "pointer", fontSize: 13, color: "#D4537E", fontWeight: 600 }}>
                      📁 내 사진 업로드
                    </button>
                    <input ref={fileRef} type="file" accept="image/*" style={{ display: "none" }} onChange={handleFile} />
                    <input style={{ ...inp, borderColor: "#F4C0D1", fontSize: 13 }}
                      placeholder="또는 이미지 URL 붙여넣기"
                      value={imgPreview ? "" : s.form.imageUrl}
                      onChange={e => { setImgPreview(null); dispatch({ type: "SET_FORM", fields: { imageUrl: e.target.value } }); }}
                    />
                    {(imgPreview || s.form.imageUrl) && (
                      <div style={{ position: "relative" }}>
                        <img src={imgPreview || s.form.imageUrl} alt="" style={{ width: "100%", maxHeight: 160, objectFit: "cover", borderRadius: 10 }} />
                        <button onClick={() => { setImgPreview(null); dispatch({ type: "SET_FORM", fields: { imageUrl: "", imageFile: null } }); if (fileRef.current) fileRef.current.value = ""; }}
                          style={{ position: "absolute", top: 6, right: 6, background: "#fff", border: "none", borderRadius: "50%", width: 24, height: 24, cursor: "pointer", fontSize: 13, fontWeight: 700, color: "#E24B4A" }}>
                          ✕
                        </button>
                      </div>
                    )}
